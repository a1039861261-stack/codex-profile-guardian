...
- `v1.10.4` 本地候选（尚未发布）：修复用户已在本地删除聊天 JSONL、但 SQLite 仍保留会话元数据时账号切换被永久阻断的问题。Guardian 现在只把受管会话目录内确实不存在的文件视为已缺失正文：保留 SQLite 行、路径、标题、归档与索引，只迁移 provider，不重建或伪造聊天文件，并在切换结果和日志中明确告警。路径越界、文件仍存在、ID 不匹配、正文分叉或归档位置冲突仍会安全中止并回滚。
...
- 当前正式发布版：`v1.10.3`；当前源码为尚未发布的 `v1.10.4` 本地候选；不可覆盖的回滚基线仍为 `v1.6.2`。
