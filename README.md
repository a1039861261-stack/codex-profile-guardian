...
- `v1.10.3` 已正式发布：当 SQLite 路径陈旧、归档标记不一致或缺少对应 thread 行时，不再把用户卡在“人工只读核对”。Guardian 会在应用内列出脱敏的副本位置、修改时间和大小，要求逐组明确选择保留项；提交时绑定检测版本，先做完整冷备，再把未选分支移入可恢复隔离库。若已有数据库行，只对齐其 `rollout_path/archived`，正文、会话 ID、标题、provider 与索引不变；失败会同时恢复文件路径和数据库引用。禁用主按钮也已改为明确灰色。
...
- 当前正式发布版：`v1.10.3`；不可覆盖的回滚基线仍为 `v1.6.2`。
