  ...
  return (
    <div className={`quota-meter is-${tone}`}>
      <div className="quota-meter-heading">
        <strong>{label}</strong>
        <span>{remaining == null ? "待同步" : `剩余 ${remaining}%`}</span>
      </div>
      <div
...
function ResetCardBalance({ data }) {
  ...
  return (
    <div className="reset-card-balance">
      <div>
        <strong>重置卡</strong>
    ...
    <div className={`official-quota ${hasQuota ? "has-data" : "is-empty"}`}>
      ...
      {hasQuota ? (
        <>
          ...
          {quota?.fetched_at && <small className="quota-live">自动同步 · {formatDate(quota.fetched_at)}</small>}
...
function Settings({ status, onSave, onOpen, onRemoteSync, busy }) {
  ...
  return (
    <div className="page-stack settings-layout">
      <section className="content-panel settings-panel">
        ...
        <Toggle checked={settings.auto_close_codex ?? true} onChange={(value) => update("auto_close_codex", value)} label="切换前自动关闭 Codex" detail="先正常退出；超时后只清理 Codex 自身的残留进程" />
        <Toggle checked={settings.auto_launch_codex ?? true} onChange={(value) => update("auto_launch_codex", value)} label="切换成功后自动启动 Codex" detail="优先使用 Microsoft Store 系统入口" />
      </section>
      <section className="content-panel settings-panel">
        ...
        <Toggle
          ...
          onChange={(value) => update("sync_ssh_official", value)}
        ...
        <Toggle
          ...
          onChange={(value) => update("sync_ssh_api", value)}
      ...
      <section className="content-panel settings-panel">
        ...
        <label className="number-setting">
          ...
          <input type="number" min="3" max="50" value={settings.backup_limit || 10} onChange={(event) => update("backup_limit", Number(event.target.value))} />
        </label>
...
function ConfirmModal({ action, status, onClose, onConfirm, busy }) {
  ...
  return (
    <Modal
      title={isRestore ? "恢复这个备份？" : isDelete ? "删除这个账号？" : isSync ? `更新 ${profile?.name} 的登录？` : isRemoteSync ? `同步 ${profile?.name} 到 SSH？` : `切换到 ${profile?.name}？`}
      description={isRestore ? "恢复会回到该时间点的配置与会话索引" : isDelete ? "只删除 Guardian 保存的加密凭据" : isSync ? "将自动关闭 Codex，并读取刚刚重新登录后的最新凭据" : isRemoteSync ? `将写入 ${action?.hostCount || 0} 台已登记 SSH 主机，并让远端 Codex 重新加载配置` : "将自动关闭 Codex 并创建配置与元数据回滚点"}
      onClose={onClose}
      size="small"
    >
      <div className={`confirm-visual ${isDelete ? "danger" : ""}`}>
        {isRestore ? <Archive weight="duotone" /> : isDelete ? <Trash weight="duotone" /> : isSync ? <ArrowClockwise weight="duotone" /> : <UserSwitch weight="duotone" />}
      </div>
      ...
      <footer className="modal-footer">
        ...
        <Button tone={isDelete ? "danger" : "primary"} icon={isDelete ? Trash : ArrowClockwise} loading={busy} onClick={onConfirm} disabled={busy}>
          {isRestore ? "确认恢复" : isDelete ? "确认删除" : isSync ? "更新登录" : isRemoteSync ? "同步到 SSH" : "安全切换"}
        </Button>
      </footer>
    </Modal>
  );
