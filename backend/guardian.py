...
class GuardianService:
    ...
    def _codex_app_server_command(self) -> list[str]:
        ...
        # The Windows Store app bundles a current native CLI even on machines
        # that never installed the npm package.
        if os.name == "nt":
            try:
                ...
                install_location = (discovered.stdout or "").strip().splitlines()
                if install_location:
                    packaged = Path(install_location[-1]) / "app" / "resources" / "codex.exe"
                    if packaged.is_file():
                        probe = subprocess.run(
