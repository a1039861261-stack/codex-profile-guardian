...
云端新虚拟机没有本机不可覆盖的 `v1.6.2` 安装包。构建的 `-IsolatedHostedRunner` 模式只接受 GitHub-hosted 环境，拒绝携带本机基线、拒绝构建 v1.6.2 或在输出目录发现该基线；manifest 明确记录基线不在云端，而不是伪称已验证云端副本。普通本地构建仍必须提供原安装包并核对固定 SHA-256。
