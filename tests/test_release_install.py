...
class ReleaseInstallTests(unittest.TestCase):
    ...
    def test_installer_has_user_run_fallback_for_denied_task_registration(self) -> None:
        install = (self.project / "installer" / "install.ps1").read_text(encoding="utf-8")
        uninstall = (self.project / "installer" / "uninstall.ps1").read_text(encoding="utf-8")
        self.assertIn("CurrentVersion\\Run", install)
        self.assertIn("if ($Create.ExitCode -eq 0)", install)
        self.assertIn("Start-Process -FilePath $SupervisorExe", install)
        self.assertIn("$PreviousRunValueExisted", install)
        self.assertIn("CurrentVersion\\Run", uninstall)
        self.assertIn("Remove-ItemProperty -LiteralPath $RunKey", uninstall)

    def test_release_build_wires_guardian_icon_into_app_and_setup(self) -> None:
        spec = (self.project / "guardian.spec").read_text(encoding="utf-8")
        installer = (self.project / "installer" / "build-installer.ps1").read_text(
            encoding="utf-8"
        )
        release = (self.project / "tools" / "build-release.ps1").read_text(encoding="utf-8")
        install = (self.project / "installer" / "install.ps1").read_text(encoding="utf-8")
        self.assertIn('icon=str(project_root / "assets" / "guardian.ico")', spec)
        self.assertIn('"--onefile"', installer)
        self.assertIn('"--windowed"', installer)
        self.assertIn('"--icon", $IconPath', installer)
        self.assertIn("installer\\build-installer.ps1", release)
        self.assertIn("tools\\installer_exe_smoke.py", release)
