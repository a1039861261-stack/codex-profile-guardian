  ...
  return (
    <section className={`health-strip ${safe ? "is-safe" : "is-warning"}`}>
      ...
      <div className="health-copy">
        ...
        <small>
          单一本机聊天库 · 数据库 {db.integrity === "ok" ? "完整" : db.integrity || "未知"} · {db.active || 0} 条活动会话 · {db.archived || 0} 条归档会话
        </small>
      </div>
      ...
      <section className={`content-panel history-conflict-panel ${activeTurnCount || uncertainTurnCount || divergentCount || lineageBlocked ? "is-warning" : "is-safe"}`}>
        ...
        <div className="history-conflict-copy">
          ...
          {conflictLoading ? (
            ...
            <><h2>未发现正文冲突，当前没有进行中任务</h2><p>账号切换时会复核任务状态，并直接强制结束确认属于 Codex 的后台进程。</p></>
          ) : (
  ...
  return (
    <div className="page-stack settings-layout">
      <section className="content-panel settings-panel">
        ...
        <Toggle checked={settings.auto_close_codex ?? true} onChange={(value) => setSetting("auto_close_codex", value)} label="切换前自动强制退出 Codex" detail="直接强制退出 Codex，无需等待正常关闭；有进行中任务时停止切换" />
      ...
      <section className="content-panel settings-panel update-panel">
        ...
        <div className="settings-footer settings-footer-split update-actions">
          ...
          <div>
            <Button icon={ArrowClockwise} onClick={onUpdateCheck} disabled={busy}>手动检查更新</Button>
  ...
  return (
    ...
    >
      <div className="conflict-confirm-warning">
        <Warning weight="fill" />
        <div><strong>不要手动删除聊天</strong><p>操作会再次核对任务并直接强制退出 Codex；所有原始副本会先进入完整冷备，未选分支只移入可恢复隔离库。</p></div>
      </div>
...
export function App() {
  ...
  const refreshQuotaNow = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      const result = await refreshOfficialQuotas();
      if (result !== null) notify("官方额度已手动同步");
    } finally {
      setRefreshing(false);
    }
