...
- `v1.10.6`：修复聊天冲突处理在完整冷备后失败却只显示通用提示、且遗漏失败日志的问题。现在会显示脱敏的失败阶段、固定原因码、诊断编号，以及冷备、聊天写入和恢复状态；失败后操作日志自动刷新，长错误码支持窄屏换行。SQLite 结构、触发器、完整性、活动任务和回滚保护全部保留，不会绕过检查或自动合并聊天；仍被拦截时，请保留冷备并提供新版的具体提示以继续排查。
- `v1.10.5`：修复“安全切换”仍需手动关闭 Codex 的正常退出流程。逐个请求已核验身份的桌面窗口正常关闭，最多等待 30 秒；兼容新版独立运行时 app-server，只等待有桌面归属的进程，不误关独立 CLI。进行中或不确定任务仍阻止切换；权限、弹窗、查询失败或退出超时会明确提示，不会强制结束进程。测试与 Windows 安装包构建现可在 GitHub 云端完成。
- `v1.10.4` 已正式发布：修复用户已在本地删除聊天 JSONL、但 SQLite 仍保留会话元数据时账号切换被永久阻断的问题。Guardian 现在只把受管会话目录内确实不存在的文件视为已缺失正文：保留 SQLite 行、路径、标题、归档与索引，只迁移 provider，不重建或伪造聊天文件，并在切换结果和日志中明确告警。路径越界、文件仍存在、ID 不匹配、正文分叉或归档位置冲突仍会安全中止并回滚。
- `v1.10.3`：当 SQLite 路径陈旧、归档标记不一致或缺少对应 thread 行时，不再把用户卡在“人工只读核对”。Guardian 会在应用内列出脱敏的副本位置、修改时间和大小，要求逐组明确选择保留项；提交时绑定检测版本，先做完整冷备，再把未选分支移入可恢复隔离库。若已有数据库行，只对齐其 `rollout_path/archived`，正文、会话 ID、标题、provider 与索引不变；失败会同时恢复文件路径和数据库引用。禁用主按钮也已改为明确灰色。
- `v1.10.2`：修复 Codex 已退出后，最近 rollout 中未收尾的 `task_started` 仍被误报为“任务正在执行”的问题。Guardian 现在只读核对 Codex 桌面端、packaged app-server、原生 CLI 与旧版 Node.js CLI 写入进程；明确没有写入进程时，把未收尾事件标记为“已中断”并允许继续安全检查，进程查询失败时仍按不确定状态停止切换。关闭范围没有扩大到普通 CLI，也不会修改会话文件来伪造完成。
- `v1.10.1`：账号切换前只读检测进行中任务和同 ID 正文分叉；检测到任务或无法完整确认最近任务状态时，不关闭 Codex，任何切换都不再强制结束进程。聊天保护页可在明确确认后执行全量哈希冷备，保留 SQLite 当前引用副本，并把其他正文分支移入可恢复隔离库；失败时恢复原路径。管理接口会返回限定的可操作错误，不再只显示通用提示。
- `v1.10.0`：官方账号可直接在 Guardian 中发起 ChatGPT OAuth 绑定。登录由官方 Codex CLI 在一次性独立 `CODEX_HOME` 中完成，不要求先切换 Codex 当前账号，不收集密码，也不修改当前 Codex 配置或聊天。“概览”已改为“主页”，主页只显示全部官方账号的会员等级、每周额度、重置卡、当前/已绑定状态和同步时间；后台与可见页面继续每 60 秒同步全部账号额度。账号页同时隐藏当前账号的无效删除入口，并调整额度卡片留白。
...
- 当前版本：`v1.10.6`；是否已正式发布以 [GitHub Releases](https://github.com/a1039861261-stack/codex-profile-guardian/releases) 为准；不可覆盖的回滚基线仍为 `v1.6.2`。
...
通过 GitHub 网页编辑 `ci/` 开头的开发分支或提交 Pull Request，`Windows release` 工作流会在 GitHub 托管的临时 Windows 环境执行全量测试、前端构建、公开扫描、打包和隔离安装验收。也可在 Actions 页面手动运行。普通提交不发布更新；只有在 `main` 手动勾选 `publish` 并且全部门禁成功，才创建新的正式 Release。

无需在日常电脑安装项目依赖，也不使用本机 runner。云端工作目录随任务结束回收，下载用的 Actions 临时成品保留 3 天，正式版本资产保留在 Releases。程序实际安装运行仍会占用本机的正常资源。

详见 [云端开发、发布与本地清理边界](docs/CLOUD-DEVELOPMENT.md)。
