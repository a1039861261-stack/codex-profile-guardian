...
- `v1.10.2`（本地候选，尚未发布）：修复 Codex 已退出后，最近 rollout 中未收尾的 `task_started` 仍被误报为“任务正在执行”的问题。Guardian 现在只读核对 Codex 桌面端、packaged app-server、原生 CLI 与旧版 Node.js CLI 写入进程；明确没有写入进程时，把未收尾事件标记为“已中断”并允许继续安全检查，进程查询失败时仍按不确定状态停止切换。关闭范围没有扩大到普通 CLI，也不会修改会话文件来伪造完成。
...
- 当前正式发布版：`v1.10.1`；不可覆盖的回滚基线仍为 `v1.6.2`。
